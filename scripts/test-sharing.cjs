...
for(const state of [{city:'san-jose',subtotal:60,tax:null,tip:0,fee:null,fulfillment:'delivery'},{city:'campbell',subtotal:81.52,tax:18.29,tip:4.53,fee:6.25,fulfillment:'pickup'}]){
...
const bad=X.plan(new URLSearchParams('city=evil&sub=-2&tax=oops&tip=Infinity&mode=other'),['san-jose']);assert.deepEqual(bad,{city:'san-jose',subtotal:60,tax:null,tip:0,fee:null,fulfillment:'delivery'});
assert.equal(new URL(X.shareURL(base,'delivery',{city:'san-jose',subtotal:1,tax:null,tip:0,fee:null,fulfillment:'delivery'})).searchParams.has('tax'),false);
