...
function cityUpdate(){const city=DATA.cities.find(c=>c.id===$('#city').value);$('#city-result').innerHTML=city?'<p><strong>'+safe(city.name)+' local context.</strong><br>City selection does not establish retailer coverage or address eligibility.<br><a href="delivery-'+city.id+'.html">Read the city notes ↗</a></p>':'<p>Select a city to explore local context. This tool does not determine service eligibility.</p>';return {city:city?.name||null,exactAddressConfirmed:false};}
...
function budgetUpdate(){
 ...
 if($('#fulfillment').value==='delivery'&&$('#delivery-fee').value.trim()==='')throw new Error('Enter the delivery fee shown on your receipt or policy, including zero.');
 const r=GuideMath.budget({subtotal:Number($('#subtotal').value),tax:$('#tax').value.trim()===''?null:Number($('#tax').value),tip:Number($('#tip').value||0),fee:$('#fulfillment').value==='pickup'?0:Number($('#delivery-fee').value),minimum:0,fulfillment:$('#fulfillment').value});
...
['subtotal','tax','tip','delivery-fee','fulfillment'].forEach(id=>$('#'+id)?.addEventListener('input',budgetUpdate));if($('#budget-result'))budgetUpdate();if($('#city-result'))cityUpdate();
$$('[data-check]').forEach(c=>c.addEventListener('change',()=>{const n=$$('[data-check]:checked').length;$('#check-progress').textContent=n===5?'5 of 5 checked. Confirm the final details with the retailer.':n+' of 5 checked';}));
...
if(document.modelContext?.registerTool){
 ...
 if($('#city'))tools.push({name:'show_city_context',description:'Select local editorial context. Does not determine service coverage or address eligibility.',inputSchema:{type:'object',properties:{cityId:{type:'string',enum:DATA.cities.map(c=>c.id).concat('other')}},required:['cityId'],additionalProperties:false},annotations:{readOnlyHint:false},execute:input=>{if(!DATA.cities.some(c=>c.id===input?.cityId)&&input?.cityId!=='other')throw new Error('Unknown city');$('#city').value=input.cityId;return cityUpdate();}});
