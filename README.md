...
Live: https://san-jose-delivery-guide.netlify.app
...
Live at https://san-jose-delivery-guide.netlify.app/ on Netlify, which publishes `dist/` on every push to `main`. Canonical URLs, the sitemap and llms.txt use the address set in `data/config.json`; change it there and run `python3 scripts/build.py` (Python 3.12+) if the site moves.
